...
public class Hopfield extends JFrame implements ActionListener {
  ...
  protected void run()
  {
    ...
    StringBuilder keyInFinal=new StringBuilder();
    ...
    AES end=new AES(keyInFinal.toString());
    String decMsg= end.decrypt(enc);
    int getRealId=getId(decMsg);
    ...
    if (getRealId!=id){
      ...
      for ( int row=0;row<NETWORK_SIZE;row++ ) {
        output[row].setBackground(java.awt.Color.red);
        output[row].setText(fail.charAt(row)+"");
      }
      return;
    }

    System.out.println("Authentication Success");
    System.out.println("key generated from HNN  = " + keyInFinal);
    System.out.println("Decrypted msg = "+getMsg(decMsg));
  }
   ...
   */
  private static StringBuilder key = null;
  String enc="";
  boolean train=false;
  int receiverUserId;
  protected void train()
  {
    ...
    if (!train) {
      ...
      String orgmsg=inputScanner.nextLine();
      System.out.println("Enter userId of receiver :");
      receiverUserId = inputScanner.nextInt();
      train=true;
      key=new StringBuilder();
      for (int i = 0; i < NETWORK_SIZE; i++) {
        key.append(input[i].getSelectedIndex());
      }
      System.out.println("Input Key = "+key.toString());
      AES aesStart= new AES(key.toString());
      enc= aesStart.encrypt(orgmsg+"*"+receiverUserId);
